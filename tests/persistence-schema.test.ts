...
describe('sqlite schema cleanup', () => {
  ...
  it('migrates legacy secret and slack-keyed tables to the cleaned schema', async () => {
    ...
    expect(tableExists(migrated, 'recurring_jobs')).toBe(false);
    expect(tableExists(migrated, 'memory_index_runs')).toBe(false);
  ...
  it('rolls back failed migrations without recording them', async () => {
    ...
    const db = new Database(sqlitePath);
    const { runMigrationList } = await import('#app/server/persistence/migrator');
