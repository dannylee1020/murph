...
vi.mock('#app/server/channels/slack/events', () => ({
  handleSlackEventEnvelope
}));

vi.mock('#app/server/channels/slack/interactions', () => ({
...
describe('SlackSocketModeClient', () => {
  ...
  it('does not start without a Slack app token', async () => {
    const { SlackSocketModeClient } = await import('../app/server/channels/slack/socket-client');
