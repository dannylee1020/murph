...
describe('scoped plugin loader', () => {
  ...
  it('loads plugin skills and read-only integration capabilities', async () => {
    ...
    const { loadScopedPlugins } = await import('#app/server/plugins/loader');
    const { loadSkills } = await import('#app/server/skills/loader');
  ...
  it('loads only the Murph home plugin root by default', async () => {
    ...
    const { getScopedPluginRoots, loadScopedPlugins } = await import('#app/server/plugins/loader');
