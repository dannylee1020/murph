...
const repoCli = join(repoRoot, 'app/team/cli/murph');
...
function createInstalledFixture() {
  ...
  symlinkSync(join(appDir, 'app/team/cli/murph'), join(binDir, 'murph'));
...
describe('murph uninstall', () => {
  ...
  it('keeps a source checkout while removing generated local files', () => {
    ...
    writeFileSync(join(murphHome, '.credentials'), '{"version":1,"credentials":[]}\n');
    writeFileSync(join(murphHome, 'config.yaml'), 'app:\n  sqlitePath: data/murph.sqlite\n');
    symlinkSync(join(sourceDir, 'app/team/cli/murph'), join(binDir, 'murph'));

    const result = spawnSync('bash', [join(sourceDir, 'app/team/cli/murph'), 'uninstall', '--yes'], {
      cwd: sourceDir,
      env: {
        ...process.env,
        HOME: home,
