...
import type { ContextAssembly, ContinuityTask, SkillManifest } from '../src/lib/types';
...
function runResult(taskInput: ContinuityTask, workspaceId: string): AgentRunResult {
...
async function setup() {
  ...
  const runSpy = vi.spyOn(AgentRuntime.prototype, 'run').mockImplementation(async (input) => runResult(input, workspace.id));
