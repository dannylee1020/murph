...
function createAppFixture() {
  ...
  mkdirSync(join(appDir, 'dist/murph-team/runtime'), { recursive: true });
  mkdirSync(join(appDir, 'dist/murph-personal/runtime'), { recursive: true });
  mkdirSync(murphHome, { recursive: true });
  mkdirSync(toolsDir, { recursive: true });
  writeFileSync(join(appDir, 'package.json'), '{"name":"murph","version":"0.0.0"}\n');
  writeFileSync(join(appDir, 'dist/murph-team/runtime/server.js'), 'require("node:fs").writeFileSync(process.env.MURPH_STARTED_FILE, process.env.MURPH_DISTRIBUTION || "");\n');
  writeFileSync(join(appDir, 'dist/murph-personal/runtime/server.js'), 'require("node:fs").writeFileSync(process.env.MURPH_STARTED_FILE, process.env.MURPH_DISTRIBUTION || "");\n');
