...
Murph uses Slack Socket Mode by default, so local development does not require a public Events URL. During setup, Murph creates the Slack app, opens the Slack approval URL, stores bot and user-search tokens in `~/.murph/.credentials`, and lets you choose watched channels. Reconnect Slack after adding new scopes so Murph can store a fresh user-search token.
