...
Plugins are installed, reloaded, and governed at the host level. The operator decides which plugin code is available to the shared runtime. Use policy, channel scope, and source configuration to limit when a host plugin is used.
