...
export function listAgentRuns(db: Db, sessionId?: string, limit = 50, workspaceId?: string, targetUserId?: string): AgentRunRecord[] {
  const where: string[] = [];
...
export function listRunSummaries(
  ...
  return listAgentRuns(db, sessionId, limit, workspaceId, targetUserId).map((run) => {
