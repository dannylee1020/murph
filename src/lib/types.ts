...
export type TaskSource = 'slack_event' | 'heartbeat' | 'review_queue';
...
export interface ContextArtifact {
  id: string;
  source: string;
  type: 'document' | 'issue' | 'meeting_note' | 'transcript' | 'link' | 'memory' | 'other';
  title: string;
  text: string;
  url?: string;
...
export interface SkillManifest {
  ...
  groundingPolicy?: 'model_choice' | 'prefer_search' | 'required_when_no_artifacts';
  channelNames?: string[];
...
export interface ChannelAdapter {
  ...
  displayName: string;
  capabilities: Array<
    'event_ingress' | 'thread_fetch' | 'reply_post' | 'message_post' | 'membership_check' | 'self_join'
  >;
  normalizeEvent(
    event: Record<string, unknown>,
    envelope?: { eventId?: string; teamId?: string }
  ): ContinuityTask | null;
  fetchThread(workspace: Workspace, thread: ChannelThreadRef): Promise<ChannelMessage[]>;
  postReply(workspace: Workspace, thread: ChannelThreadRef, text: string): Promise<void>;
  postMessage?(workspace: Workspace, channelId: string, text: string): Promise<{ ts?: string }>;
  checkMembership?(workspace: Workspace, channelId: string): Promise<ChannelMembershipStatus>;
  ensureMember?(workspace: Workspace, channelId: string): Promise<ChannelEnsureMemberResult>;
}
...
export interface ChannelEnsureMemberResult {
  channelId: string;
  name?: string;
  status: 'already_member' | 'joined' | 'requires_invitation' | 'reinstall_required' | 'error';
  reason?: string;
}
...
export type CapabilityStatus = 'loaded' | 'disabled' | 'misconfigured' | 'failed';
