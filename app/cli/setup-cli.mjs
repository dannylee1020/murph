...
function runMurphCommand(args) {
  const binPath = existsSync(path.join(appDir, 'bin', 'murph'))
    ...
    : path.join(appDir, 'app', 'cli', 'murph');
...
async function ensureServer() {
  if (await health()) return;
  const runtimeEntry =
    path.join(appDir, 'dist', 'app', 'runtime', 'server.js');
...
async function setupPolicy() {
  ...
  success(`Saved policy: ${selected.name} / ${selected.compiled?.executionMode || 'manual_review'}`);
}
